function parseOffers(txt) {
    ...
    return content
        ...
        .map(block => {
            ...
            if (!offer.title || !offer.image || !offer.description) {
                return null;
            }
    ...
    return `
        <div class="card">
            ...
            <div class="card__content">
                ...
                <span class="card__hint card__hint--close">
...
function renderOffers(offers) {
    ...
    for (let i = 0; i < offers.length; i += 2) {
        ...
        grid.innerHTML =
            renderOfferCard(offers[i]) +
            (offers[i + 1] ? renderOfferCard(offers[i + 1]) : "");
...
function loadOffers() {
    fetch("data/offers.txt")
        .then(response => {
            if (!response.ok) {
                throw new Error("Failed to load offers.txt");
            }
