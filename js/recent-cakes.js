document.addEventListener("DOMContentLoaded", async function () {
    ...
    try {
        const response = await fetch("images/recent-cakes/index.json", { cache: "no-cache" });
        if (!response.ok) throw new Error("Failed to load recent-cakes index");
        ...
        function buildSet() {
            return images.map(function (filename) {
                ...
                img.style.setProperty("--tilt", randomTilt() + "deg");
                return img;
            });
        }

        track.innerHTML = "";
        buildSet().forEach(img => track.appendChild(img));
        buildSet().forEach(img => track.appendChild(img));
...
function openRecentCakesGallery() {
    ...
    content.querySelectorAll(".recent-gallery-img").forEach(img => img.remove());
